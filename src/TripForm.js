...
export default function TripForm() {
  ...
  const api_key = "AIzaSyAAcn9FpPAv1ukFI8SBrFrwG9K8YJSg_8Y";
  console.log("hello");
    ...
    try {
      ...
      const payload = {
        contents: [{
          role: "user",
          parts: [{
            text: `Plan a great trip to ${formData.destination} from ${formData.startDate.format('YYYY-MM-DD')} to ${formData.endDate.format('YYYY-MM-DD')}. There will be ${formData.noOfTravellers} travelers. Please provide the response in this format: ${structureData}`
          }]
        }],
      };
      ...
      const res = await axios.post(
        `https://generativelanguage.googleapis.com/v1beta2/models/geminipro:generateContent?key=${api_key}`,
        payload,
        { headers }
      );
      console.log("hello3");
      ...
      setResponse(JSON.stringify({ error: "Error fetching response from Gemini API" }));
    }
  };
  ...
  return (
    <Box component="form" noValidate autoComplete="off">
      ...
      {response && <TravelDetails tripDetails={response} />}
    </Box>
  );
}
